...
void testJumble() {
	...
	cout << "Testing Simple Constructor (easy):" << endl;
	JumblePuzzle test1("HELLO", "easy");
	showJumble(test1.getJumble(), test1.getSize());
	cout << "Size should be 10: " << test1.getSize() << endl;
	cout << "Testing Simple Constructor (medium):" << endl;
	JumblePuzzle test2("HELLO", "medium");
	showJumble(test2.getJumble(), test2.getSize());
	cout << "Size should be 15: " << test2.getSize() << endl;
	cout << "Testing Simple Constructor (hard):" << endl;
	JumblePuzzle test3("HELLO", "hard");
	showJumble(test3.getJumble(), test3.getSize());
	cout << "Size should be 20: " << test3.getSize() << endl;

	// Test constructor for invalid input handling
	cout << "Testing constructor:" << endl;
	cout << "Should see 4 errors (output to cout to eliminate asynchronous error stream):" << endl;
	JumblePuzzle* test4; // will show up as unused
	try {
		test4 = new JumblePuzzle("WE", "easy");
	} catch (BadJumbleException& e) {
		cout << e.what() << endl;
	}
	try {
			test4 = new JumblePuzzle("THISWORDBAD", "easy");
		} catch (BadJumbleException& e) {
			cout << e.what() << endl;
		}
	try {
		test4 = new JumblePuzzle("+++", "easy");
	} catch (BadJumbleException& e) {
		cout << e.what() << endl;
	}
	try {
		test4 = new JumblePuzzle("test", "impossible");
	} catch (BadJumbleException& e) {
		cout << e.what() << endl;
	}
	cout << endl;
